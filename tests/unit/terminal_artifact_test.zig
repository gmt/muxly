...
test "tty leaf can freeze into captured text artifact while preserving provenance" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, xml.items, "contentFormat=\"plain_text\"") != null);
}

test "tty leaf can freeze into captured surface artifact while preserving provenance" {
    var document = try muxly.document.Document.init(std.testing.allocator, 1, "artifact-demo");
    defer document.deinit();
    ...
    const node_id = try document.appendNode(
        ...
        "surface",
        .{ .tty = .{
            .session_name = @constCast("demo-surface"),
            .window_id = @constCast("@2"),
            .pane_id = @constCast("%4"),
        } },
    );
    ...
    switch (node.source) {
        .terminal_artifact => |artifact| {
            ...
            try std.testing.expectEqualStrings("%4", artifact.pane_id.?);
        },
        else => return error.UnexpectedSourceKind,
    }
