...
pub fn parse(allocator: std.mem.Allocator, text: []const u8) !Parsed {
    ...
    if (std.mem.indexOf(u8, payload, absolute_document_separator)) |doc_sep| {
        authority_and_path = payload[0..doc_sep];
        document_path_text = payload[doc_sep + absolute_document_separator.len - 1 ..];
    }
...
pub fn parsedTransportSpec(allocator: std.mem.Allocator, parsed: Parsed) ![]u8 {
    ...
    const scheme = switch (parsed.transport_code) {
        ...
        .ht2 => "https2",
        .ht1 => "https1",
        .auto, .ht3 => return error.UnsupportedResourceTransport,
    };
...
pub fn renderCaddyfile(
    ...
    return try std.fmt.allocPrint(
        ...
        \\}}
        \\
        \\# Note: for plain `trds://...` and `trds://wtp|...`, this generated
...
pub fn renderUserMuxlydUnit(
    ...
    return try std.fmt.allocPrint(
        allocator,
        \\# Generated by muxly for {s}
        \\# For plain `trds://...` and `trds://wtp|...`, this unit still
