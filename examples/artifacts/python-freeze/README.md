...
This example uses `ctypes` against `libmuxly` to exercise the public terminal
...
- default to `/tmp/muxly-example-python-freeze.sock` unless `MUXLY_SOCKET` is set
- start `muxlyd` if nothing is listening on that socket
- run the `ctypes`-based freeze demo
...
- `MUXLY_SOCKET`
  ...
  `/tmp/muxly-example-python-freeze.sock`.

## Intent

This playbook exists to show that a downstream `libmuxly` consumer can:

- create live tmux-backed state
- freeze tty-backed nodes through the public handle API
- inspect `artifactKind` and `contentFormat`
- interpret `sectioned_text` surface payloads without reaching back into CLI-only paths
